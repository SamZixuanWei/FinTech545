...
returns = returns.drop(index=0).reset_index(drop=True)

print(returns)
ew_return = exp_weighted_cov(returns)
print(pd.DataFrame(ew_return))
...
n = 1000
sigma = np.matrix(np.full((n, n), 0.95))
...
print(pd.DataFrame(direct_simulation(ew_return)))
print(pd.DataFrame(pca_simulation(ew_return,pct_explained=0.9)))


# In[96]:


data = np.genfromtxt('DailyReturn.csv', delimiter=',').T
data = np.delete(data, 0, 0)
data = np.delete(data, 0, 1)
data = np.matrix(np.fliplr(data))
sample_return = (data[0] - data[0].mean()).T
var_norm = normal_var(sample_return)
print(var_norm)
var_t = t_var(sample_return)
print(var_t)
var_hist = historic_var(sample_return)
print(var_hist)
